...
public:
   ...
   bloom_filter(const std::size_t& table_size, const std::size_t& salt_count,
                const std::size_t& random_seed, const unsigned char* buffer)
   ...
   inline bloom_filter& operator = (const bloom_filter& f)
   {
      if (this != &f)
      {
         ...
         bit_table_ = new cell_type[table_size_ / bits_per_char];
         std::copy(f.bit_table_,f.bit_table_ + (table_size_ / bits_per_char),bit_table_);
         salt_       = f.salt_;

         predicted_element_count_ = f.predicted_element_count_;
   ...
   bool operator== (const bloom_filter& filter)
   {

	   //if(table_size_ == filter.table_size_){NS_LOG_UNCOND("table_size_ OK");}
     if (salt_count_ == filter.salt_count_ && table_size_ == filter.table_size_ && predicted_element_count_ == filter.predicted_element_count_
        && random_seed_ == filter.random_seed_  && desired_false_positive_probability_ == filter.desired_false_positive_probability_)
        {
			for (std::size_t i=0; i<table_size_/ bits_per_char;i++){
			  if (filter.bit_table_[i]!=bit_table_[i]){
				return false;
			  }
			}//for
			return true;
          //return memcmp(filter.bit_table_, bit_table_, table_size_) == 0;
            //NS_LOG_UNCOND("going to return true");
   ...
   inline void assign_bitTable(unsigned char *bitTable, uint16_t tableSize)
   {
      bit_table_ = new cell_type[(size_t)tableSize / bits_per_char];
      std::copy (bitTable, bitTable + ((size_t)tableSize / bits_per_char),bit_table_);
   }

   inline void set_salt_count (uint16_t saltCount)
   {
        salt_count_ = (size_t)saltCount;
   }
   ...
   inline void insert(const unsigned char* key_begin, const std::size_t& length)
   {
      ...
      for(std::vector<bloom_type>::iterator itr = salt_.begin(); itr != salt_.end(); ++itr)
      {
         ...
         //NS_LOG_UNCOND("bit_index / bits_per_char = "<<bit_index / bits_per_char<<"    and bit = "<<bit<<"    and bit_mask[bit] = "<<bit_mask[bit]);
   ...
   inline void set_inserted_element_count (std::size_t iec)
   {
     inserted_element_count_=iec;
   }
   ...
   {
      /* union */
      unsigned char* bit_table_2;
      bit_table_2 = new cell_type[table_size_ / bits_per_char];
      std::copy(bit_table_, bit_table_ + (table_size_ / bits_per_char), bit_table_2);
      //NS_LOG_UNCOND("bit_table_ before union= "<<(uint16_t)(*bit_table_));
      if (
          (salt_count_  == filter.salt_count_) &&
          (table_size_  == filter.table_size_) &&
          (random_seed_ == filter.random_seed_)
      ...
      {
		 // NS_LOG_UNCOND("oomad filtera ro OR kone");
         for (std::size_t i = 0; i < (table_size_ / bits_per_char); ++i)
         {
            bit_table_[i] |= filter.bit_table_[i];
         }
      }
      //NS_LOG_UNCOND("filter.bit_table_ = "<<(uint16_t)(*filter.bit_table_) <<"    and bit_table_ after union= "<<(uint16_t)(*bit_table_));
      for (std::size_t i=0; i<table_size_ /bits_per_char;i++){
		  if (bit_table_[i] != bit_table_2[i]){
	        //NS_LOG_UNCOND("the filter has changed after union. Inequality at bit "<<i);
            break;
		  }
	  }
	  delete bit_table_2;
